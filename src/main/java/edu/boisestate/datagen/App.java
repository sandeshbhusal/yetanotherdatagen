...
public class App {
    ...
    public static void main(String[] args) {
        ...
        argParser.addArgument("-x", "--skip_invariant_gen").help(
                "Skip generating invariants from the generated data. User will be responsible for analyzing data later.")
                .required(false)
                .setDefault(true)
                .type(Boolean.class);

        argParser.addArgument("-i", "--iteration_count").help(
        ...
        do {
            ...
            for (String key : traces.keySet()) {
                ...
                if (!skipInvariantGeneration) {
                    runDaikonOnDtraceFile(
                            new File(codePath + "/" + key + ".dtrace"),
                            classpaths,
                            new File(codePath + "/" + key + ".daikonoutput"));
                } else {
                    Logger.info("Skipping daikon invariant generation but trace file is written.");
                }
            }
            ...
            for (String key : dig_traces.keySet()) {
                ...
                if (!skipInvariantGeneration) {
                    ...
                    Logger.info("Skipping DIG invariants generation, but trace file is written.");
                }
            }

            /// Note to whoever is looking at this later:
            /// Initially, we started out with a "stability check", i.e. if the invariants
            /// do not
            /// change within a certain window, we stop iterations. However, this proved
            /// unnecessary, as
            /// programs generally stabilize around 20 iterations anyways.
            ///
