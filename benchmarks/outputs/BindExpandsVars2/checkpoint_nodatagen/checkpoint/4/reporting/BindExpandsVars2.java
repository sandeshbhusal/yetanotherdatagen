...
public class BindExpandsVars2 {
    ...
    public static void test(int cp1_off, int n1, int n2, int MAXDATA) {
        ...
        if (n2 > MAXDATA * 2 - n1)
            return;
        Report.datagen_instrument(13019, "startingconds", "cp1_off", cp1_off, "n1", n1, "n2", n2, "MAXDATA", MAXDATA);
        Report.datagen_guard_instrument(13019, "funcguard", "cp1_off", cp1_off, "n1", n1, "n2", n2, "MAXDATA", MAXDATA);
        int mc_i;
        for (mc_i = 0; mc_i < n2; mc_i++) {
            Report.datagen_instrument(13019, "loopinvariant", "mc_i", mc_i, "cp1_off", cp1_off, "MAXDATA", MAXDATA);
        }
    }
}
