...
if __name__ == "__main__":
    ...
    for test in tests:
        print(test, ":", prattparser(lexer(test)))

    print("==== invariant tests ====")
    invariant_tests = []
    for test in tests:
        for test2 in tests:
            if "mod" in test or "mod" in test2 or test == test2:
                continue
            invariant_tests.append(f"{test} == {test2}")
            invariant_tests.append(f"{test} != {test2}")
            invariant_tests.append(f"{test} < {test2}")
            invariant_tests.append(f"{test} > {test2}")
            invariant_tests.append(f"{test} <= {test2}")
            invariant_tests.append(f"{test} >= {test2}")
    # Special test for mod.
    invariant_tests.append("x1 + x2 === 5 (mod 12)")

    for invtest in invariant_tests:
        print(invtest, ":", end=" ")
        inv = invariant_to_s_expr(invtest)
        print(inv[1], end=" with atoms as: ")
        print(inv[0])
